...
The methods we mentioned to overcome multicollinearity include:  
  ...
  - Variable selection:
    - by hand: based on theory, only select relevant variables.
    - by models: elasticnet (lasso and ridge)
...
Trees might use different criteria than R2 to select the most important variables. 
...
Even though it is multivariate model, it runs bivariate analysis each time such that there is no multicollinearity issues. 

But if it is only running bivariate regression each time, there would be no control variables. So, the effect of that variable might not be due to only that variable itself ???
...
Above figure shows overfitting.
...
There will be 2 main hyperparameters that we will fine-tune:  
  ...
  Example: p-value = 0.05 so (1-0.05) = 0.95
...
Cons:  
  - Can have higher bias (easy to overfit) 
  ...
  Even though there is variability in the boxplot in the terminal mode, the prediction is the mean. 
...
> Elastic nets can only be run for a numerical response variable. CITs can be run with either numerical (regression) or categorical (classification) explanatory variable. Therefore, we have the need to specify the mode (regression or classifiation) here.
...
Larger rmse in the test set, lower r2 in test set. Because this test data has not been seen by the model before.
...
Because when we applied the model for train set, we have 4 terminal nodes. 
...
```
Only 2 variables are important. Why?
We have Minimum criterion = 0.999 So, 1 - 0.999 = 0.001. So only variables with p-value <= 0.001 are selected.
...
This CIT model is highly sensitive to the data supplied to it. 
